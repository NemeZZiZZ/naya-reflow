...
import { Bluetooth, CircleDot, Disc, PanelLeft, PanelRight } from 'lucide-react';
...
export default function HalfPill({ info }: { info: HalfInfo }) {
  ...
  const pctTxt = !info.connected ? '—' : shownPct === null ? '?' : `${shownPct}%`;
  const ModIcon =
    !info.connected || !info.modPresent ? null : info.modType === 'Track' ? CircleDot : Disc;
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          className={
            'relative flex cursor-pointer items-center gap-1.5 overflow-hidden rounded-full border px-3 py-1 text-xs font-semibold ' +
            (info.connected
        ...
        >
          {info.connected && shownPct !== null && (
            <span
              className="absolute inset-y-0 left-0 bg-green-600/25"
              style={{ width: `${shownPct}%` }}
            />
          )}
          <span className="relative flex items-center gap-1.5">
            <HalfIcon className="h-3.5 w-3.5" />
      ...
      <PopoverContent align="end">
        {!info.connected ? (
          ...
          <div className="grid grid-cols-[130px_1fr] gap-x-2.5 gap-y-1.5 text-[13px]">
            <Row k="Device Type" v={side === 'left' ? 'Left half' : 'Right half'} />
            ...
            <Row
              k="Module Battery"
              v={
                !info.modPresent || info.modPctVal === null ? '—' : `${info.modPctVal}%`
              }
            />
            <Row
              k="Module Voltage"
              v={!info.modPresent || info.modMv === null ? '—' : `${info.modMv} mV`}
            />
            <Row
              k="Updated"
              v={
