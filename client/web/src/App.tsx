...
import { Download, Info, Loader2, Plug, PlugZap, RefreshCw, Terminal, Trash2, Wifi, WifiOff, X } from 'lucide-react';
...
const TH = 'text-left px-2 py-1 text-muted-foreground font-semibold border-b border-border';
const TD = 'px-2 py-1 border-b border-border tabular-nums';
const HEX = 'px-2 py-1 border-b border-border tabular-nums font-mono text-xs text-[#9ecfff]';
...
async function readAux(
  ...
  const nm = await q('be/1006 BLE name', 0xbe, 0x10, 0x06, (p) => bleName(p) + '  [' + toHex(p) + ']');
  ...
  const d1 = await q('de/1001 module', 0xde, 0x10, 0x01, (p) => {
    ...
    return m ? `${m.present ? 'present' : 'absent'} ${m.type}  [${toHex(p)}]` : toHex(p);
  });
  if (d1) {
  ...
  if (modPresent) {
    ...
    if (side === 'left') {
      const d8 = await q('de/1008 module FW', 0xde, 0x10, 0x08, (p) => modFwText(p) ?? toHex(p));
...
export default function App() {
  ...
  const [keyRecs, setKeyRecs] = useState<KeyRec[]>([]);
  const [lastDump, setLastDump] = useState<{ layer: number; recs: KeyRec[] } | null>(null);
  ...
  const keymap = useMemo(() => {
    ...
    for (const r of keyRecs) if (!m.has(r.kk)) m.set(r.kk, r.rec);
    return m;
  }, [keyRecs]);
  ...
  function log(cls: string, msg: string) {
    ...
    const cat: LogEntry['cat'] = cls === 'tx' || cls === 'rx' ? 'cdc' : cls === 'err' ? 'err' : 'info';
    setLogs((prev) => {
  ...
  const onFrame: FrameHandler = (dir, bytes) =>
    log(dir === '>' ? 'tx' : 'rx', (dir === '>' ? 'TX ' : 'RX ') + toHex(bytes));
  ...
  useEffect(() => {
    ...
    const update = () => {
      setKbScale(Math.max(0.25, Math.min(1, box.clientWidth / kbNaturalW.current)));
    };
    update();
  ...
  function noteModule(side: Side, present: boolean, type: string): string | null {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (busyRef.current || sweepRef.current || sesRef.current.size === 0) return;
      void (async () => {
        ...
        for (const [side, ses] of sesRef.current) {
          ...
          try {
            ...
            setHalves((prev) =>
              prev[side].connected ? { ...prev, [side]: { ...prev[side], ...aux } } : prev,
            );
            ...
            log('inf', `aux refresh ${side} failed (${(e as Error).message}), keeping last known`);
          } finally {
            ses.onFrame = saved;
          }
        }
          ...
          try {
            ...
            setHalves((prev) =>
              ...
              (prev[side].modPresent !== m.present || prev[side].modType !== mt)
                ? { ...prev, [side]: { ...prev[side], modPresent: m.present, modType: mt } }
                : prev,
            );
          } catch {
  ...
  async function autoFetch(ses: NayaSession) {
    busyRef.current = true;
    try {
      log('inf', 'auto-fetch: layer 0 + LED map 0…');
      await ses.handshake();
      const blob = await ses.readLayer(0);
      const parsed = parseLayer(blob);
      setLastDump({ layer: 0, recs: parsed.recs });
      setKeyRecs(parsed.recs);
      setDumpStat(`${parsed.total}B blob, ${parsed.recs.length} records`);
      const lblob = await ses.readLedmap(0);
      const lparsed = parseLedmap(lblob);
      setLastLed({ layer: 0, recs: lparsed.recs });
      setLedRecs(lparsed.recs);
      setLedDumpStat(`${lparsed.total}B blob, ${lparsed.recs.length} LEDs`);
      log('inf', `auto-fetch done: ${parsed.recs.length} keys, ${lparsed.recs.length} LEDs`);
    } catch (e) {
      log('err', 'auto-fetch: ' + ((e as Error).stack ?? (e as Error).message));
    } finally {
      busyRef.current = false;
    }
  }
  ...
  async function connectPort(port: SerialPort, hint: Side) {
    ...
    try {
      ...
      ses = await NayaSession.connect(port, hint === 'left' ? DST_LEFT : DST_RIGHT, onFrame);
      ...
      const side: Side = dev.payload.length >= 27 ? 'left' : 'right';
      if (side !== hint)
        log('inf', `port identifies as ${side} (fa/1001 ${dev.payload.length}B) — assigned there`);
  ...
  function flash() {
    ...
    if (!lastDump || lastDump.layer !== 0) {
      setFlashStat('dump Layer 0 first');
      return;
    }
    const kk = parseInt(fKK, 16);
    const hid = parseInt(fHID, 16);
    if (isNaN(kk) || kk < 0 || kk > 0x9b || isNaN(hid) || hid < 0 || hid > 0xff) {
  ...
  async function doFlash() {
    ...
    busyRef.current = true;
    try {
      log('inf', `flash: KK ${hex2(plan.kk)} [${plan.curMean}] → [${plan.newMean}]`);
      setFlashStat('handshake…');
      await ses.handshake();
      setFlashStat('writing…');
      const rec = new Uint8Array([plan.kk, 0x01, 0x04, plan.hid, 0x00, 0x07, 0x00]);
  ...
  async function exportKeys(all: boolean) {
    ...
    try {
      ...
      for (const L of layers) {
        ...
        const { recs, total } = parseLayer(blob);
        (out.layers as Record<string, unknown>)[String(L)] = {
          totalBytes: total,
          records: recs.map((r) => ({ kk: r.kk, kkHex: hex2(r.kk), t: r.t, rec: toHex(r.rec), meaning: describeRecord(r.rec) })),
        };
        log('inf', `export: layer ${L}: ${total}B, ${recs.length} records`);
      }
      saveJson(all ? 'naya-left-keymap-all.json' : `naya-left-keymap-L${layer}.json`, out);
      log('inf', 'export: keymap JSON saved');
    } catch (e) {
      log('err', 'export keys: ' + ((e as Error).stack ?? (e as Error).message));
    } finally {
      busyRef.current = false;
    }
  }
  ...
  async function exportLeds(all: boolean) {
    ...
    try {
      ...
      for (const L of layers) {
        const blob = await ses.readLedmap(L);
        const { recs, total } = parseLedmap(blob);
        (out.layers as Record<string, unknown>)[String(L)] = {
          totalBytes: total,
  ...
  function ledSet() {
    ...
    if (isNaN(kk) || kk < 0 || kk > 0x87 || isNaN(h) || h < 0 || h > 511 || isNaN(s) || s < 0 || s > 255) {
  ...
  async function doColorSet() {
    ...
    busyRef.current = true;
    try {
      log('inf', `color: KK ${hex2(plan.kk)} H${plan.curH}/S${plan.curS} → H${plan.h}/S${plan.s}`);
      ...
      if (back && back.h === plan.h && back.s === plan.s) {
        ...
        log('err', 'color readback: ' + (back ? `H${back.h}/S${back.s}` : 'KK missing'));
      }
    } catch (e) {
  ...
  function onSelect(pos: number, kk: number) {
    ...
    setFKK(hx);
    setCKK(hx);
    setSelInfo(`selected pos ${pos} (${POS_KEY[String(pos)] ?? '?'}) = KK 0x${hx}`);
      ...
      <div className="flex min-h-screen">
        <div className="mx-auto w-full max-w-6xl flex-1 p-6">
          ...
          <p className="mb-4 text-[13px] text-muted-foreground">
            NayaFlow replacement over WebSerial — Chromium only (Chrome / Edge / Opera).
            Close NayaFlow first: ports open exclusively.
          </p>

          <Card className="mb-3">
            <CardContent className="flex flex-wrap items-center gap-x-3 gap-y-2 pt-4">
              <HalfPill info={halves.left} />
              {leftOn ? (
                <Button variant="secondary" size="sm" onClick={() => void disconnect('left')}>
                  <PlugZap /> Disconnect
                </Button>
              ) : connecting.left ? (
                ...
                <Button size="sm" onClick={() => void connectPicked('left')}>
                  <Plug /> Connect L…
                </Button>
              )}
              <Separator orientation="vertical" className="h-6" />
              <HalfPill info={halves.right} />
              {rightOn ? (
                <Button variant="secondary" size="sm" onClick={() => void disconnect('right')}>
                  <PlugZap /> Disconnect
                </Button>
              ) : connecting.right ? (
                <Button size="sm" disabled>
          ...
          <Card className="mb-3">
            <CardHeader>
              ...
              <span className="font-mono text-xs text-muted-foreground">{selInfo}</span>
            </CardHeader>
            <CardContent>
          ...
          <Card className="mb-3">
            ...
            <CardContent>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                <Tabs value={String(layer)} onValueChange={(v) => setLayer(parseInt(v, 10))}>
                  <TabsList>
                ...
                <Button variant="secondary" onClick={() => void dump()} disabled={!leftOn}>
                  Dump layer
                </Button>
          ...
          <Card className="mb-3">
            <CardHeader>
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  Writes one key record and applies it instantly — no fe/100a commit is ever
                  sent. Persists across reboot.
                </TooltipContent>
              </Tooltip>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap items-center gap-2">
                <Label>
                  KK <Input className="w-16" value={fKK} onChange={(e) => setFKK(e.target.value)} />
                </Label>
                <Label>
                  HID <Input className="w-16" value={fHID} onChange={(e) => setFHID(e.target.value)} />
                </Label>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Builds a T01 record [KK,01,04,HID,00,07,00] on Layer 0. Requires a fresh
                Layer-0 dump above (same-length guard refuses 7B↔11B changes — the device
                ignores those). LEFT half only.
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="mb-3">
            <CardHeader>
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  Per-layer color store (136 LEDs, KK 0x00–0x87). Applies instantly, persists
                  across reboot.
                </TooltipContent>
              </Tooltip>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <HexColorPicker
                    ...
                    color={hsToHex(isNaN(hNum) ? 0 : hNum, isNaN(sNum) ? 0 : sNum)}
                    onChange={(hex) => {
                ...
                <div className="flex min-w-52 flex-1 items-center gap-2">
                  <Label className="shrink-0">
                    H <span className="font-mono">{isNaN(hNum) ? '—' : hNum}</span>
                  </Label>
                  <Slider
                    min={0}
                    max={511}
                ...
                <div className="flex min-w-44 flex-1 items-center gap-2">
                  <Label className="shrink-0">
                    S <span className="font-mono">{isNaN(sNum) ? '—' : sNum}</span>
                  </Label>
                  <Slider
                    min={0}
                    max={255}
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Factory amber ≈ H38/S100; typical S 0–100. The picker drives the same
                H/S fields (hue wraps past 360° — type large H manually if needed).
                Requires a fresh dump of the same layer (guard). LEFT half only.
              </p>
            </CardContent>
          </Card>

          <p className="text-xs text-muted-foreground">
            Scope: device info + keymap dump/flash + LED map dump/set. No fe/100a commit is
            ever sent. Remap + LED commands answer on the LEFT half only. Battery pills
            Module presence polls every 1s (single de/1001); battery + module
            state refresh quietly every 30s; module dock/undock lands in the log as a module
            event. Log shows info + errors by default, CDC frames via the CDC chip.
          </p>
        </div>

        {logOpen && (
          <div className="sticky top-0 flex h-screen w-[min(430px,92vw)] shrink-0 flex-col border-r border-border bg-card">
            <div className="flex items-center gap-1 border-b border-border px-3 py-2">
              ...
              {(['cdc', 'info', 'err'] as const).map((c) => (
                <button
                  ...
                  title={c === 'cdc' ? 'CDC frames' : c === 'info' ? 'info lines' : 'errors'}
                  className={
            ...
            >
              {logs.filter((l) => logCats[l.cat]).map((l) => (
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetContent>
            ...
            <Tabs value={sheetTab} onValueChange={(v) => setSheetTab(v as Side)}>
              <TabsList>
            ...
            <p className="text-xs text-muted-foreground">
              Device Name / Hardware ID live in the NayaCore host app and are not readable
              over CDC. Module % is estimated from rail voltage.
            </p>
          </SheetContent>
        </Sheet>

        <Dialog open={flashPlan !== null} onOpenChange={(o) => !o && setFlashPlan(null)}>
          <DialogContent>
            <DialogTitle>Flash KK {flashPlan && hex2(flashPlan.kk)}?</DialogTitle>
            <DialogDescription>
              Writes a T01 record on Layer 0 of the LEFT half. Applies instantly, persists
              across reboot.
            </DialogDescription>
            {flashPlan && (
        ...
        <Dialog open={colorPlan !== null} onOpenChange={(o) => !o && setColorPlan(null)}>
          <DialogContent>
