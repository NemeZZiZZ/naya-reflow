...
SUB = re.compile(r'sub\s+w(8|9|21|22|23),\s*w(20|22),\s*#0x([0-9a-f]+)')
ADD = re.compile(r'add\s+w(8|9|21),\s*w(20|22),\s*#0x([0-9a-f]+)')
MOVV = re.compile(r'mov\s+w(8|9|21),\s*#0x([0-9a-f]+)')
MOVKV = re.compile(r'movk\s+w(8|9|21),\s*#0x([0-9a-f]+),\s*lsl\s*#(\d+)')
MOVR = re.compile(r'mov\s+w(8|9|21),\s*w(20|22)\b')
STRW = re.compile(r'str\s+w(8|9|20|21|22|23),\s*\[sp,')
...
for i in range(lo - 1, hi):
    ...
    if m:
        base = w20 if m.group(2) == '20' else w22
        if base is not None:
            last_alu = (m.group(1), base + int(m.group(3), 16))
            emit(last_alu[1])
        continue
    m = MOVV.search(ln)
    if m:
